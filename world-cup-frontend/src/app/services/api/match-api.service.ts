...
export class MatchApiService {
  ...
  getMatchesByDate(date: string): Observable<Match[]> {
    return this.cached(`matches:${date}`, CACHE_TTL.scheduledMatches, () =>
      this.http.get<Match[]>(`${API_ENDPOINTS.worldcup}/matches/date/${date}`, { context: this.silentContext() }).pipe(catchError(() => of(MOCK_MATCHES)))
    );
  }

  getLiveMatches(): Observable<Match[]> {
    return this.cached('live-matches', CACHE_TTL.liveMatches, () =>
      this.http.get<Match[]>(`${API_ENDPOINTS.worldcup}/matches/date/${new Date().toISOString().slice(0, 10)}`, { context: this.silentContext() }).pipe(catchError(() => of(MOCK_MATCHES.filter((m) => m.status === 'LIVE'))))
    );
  }

  getMatch(id: number): Observable<Match> {
    return this.http.get<Match>(`${API_ENDPOINTS.worldcup}/matches/${id}`, { context: this.silentContext() }).pipe(
