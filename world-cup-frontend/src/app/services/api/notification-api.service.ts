...
export class NotificationApiService {
  ...
  getUnreadNotifications(userId: number): Observable<NotificationDTO[]> {
    return this.http
      .get<
        NotificationDTO[]
      >(`${API_ENDPOINTS.notifications}/user/${userId}/unread`, { context: this.silentContext() })
      .pipe(catchError(() => of([])));
  }
  ...
  getUserNotifications(userId: number): Observable<NotificationDTO[]> {
    return this.http
      .get<
        NotificationDTO[]
      >(`${API_ENDPOINTS.notifications}/user/${userId}`, { context: this.silentContext() })
      .pipe(catchError(() => of([])));
  }

  markAsRead(id: number): Observable<void> {
    return this.http
      .put<void>(`${API_ENDPOINTS.notifications}/${id}/read`, {}, { context: this.silentContext() })
